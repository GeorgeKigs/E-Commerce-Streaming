...
const { statics } = shortCodeSchema;

shortCodeSchema.pre("save", function (next) {
	var details = db.findOneAndUpdate({ email: this.email }, { code: this.code });
	if (!details) {
...
statics.verifyByEmail = async function (
	email: string,
	code: string
): Promise<boolean> {
...
statics.verifyByPhoneNumber = async function (
	phoneNumber: number,
	code: string
): Promise<boolean> {
...
var db = model<shortCodeInt, shortCodeMethodInt>(
	"Short_Codes",
	shortCodeSchema
);
