...
var { validData } = require("./utils/testUser");
...
describe("testValidity", async function () {
	...
	it("lastName is has a valid value", async () => {
		...
		expect(result.lastName, "the last name should be a string").to.be.a(
			"string"
		);
	});
...
describe("testInvalidity", async function () {
	it("firstName is required.", async () => {
		try {
			...
			expect(
				error.errors.firstName.kind,
				"the first Name is required"
			).to.be.equal("required");
		}
	});
...
describe("testing statics", async () => {
	it("test the find by email function", async () => {
		...
		await setDB(data);
		var results = await userModel.findByEmail(data.email);
		try {
			let expected_keys = [
				"registered",
				"_id",
	...
	it("test the find by phone Number function", async () => {
		...
		await setDB(data);
		var results = await userModel.findByPhoneNumber(data.phoneNumber);
		try {
			let expected_keys = [
				"registered",
				"_id",
